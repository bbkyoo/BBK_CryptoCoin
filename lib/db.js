...
async function findAllBy(_searchFilter, _filterValue){
    const findData = await Wallet.findAll({
        where: { [_searchFilter]: _filterValue }});
    if(findData == null) throw error;
    return findData
}
...
async function deleteBy(_searchFilter, _filterValue){
    try {await Users.destroy({ where: { [_searchFilter]: _filterValue }}); }
...
async function createBy(_signData){
    try {await Users.create({
     ...
     return await findBy(['user_id'], 'name', _signData.name)
    }
...
module.exports.FindAllData = findAllBy
